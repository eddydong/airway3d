...
export function reportPDF(report) {
  ...
  para(`Fixed posture response assumptions: response factor ${number(1-Math.exp(-settings.elapsed/Math.max(.1,settings.tau)),4)}; dependent swelling L ${settings.responseL} / R ${settings.responseR} mm; immediate displacement ${settings.gravity} mm; cycle bias ${settings.cycle} mm.`);
  ...
  para('Swelling = (vascular + signed cycle bias)*fixed response factor + immediate displacement. The fixed factor is retained from the reference recording metadata; the viewer has no time-after-turning control. Vascular response is +amplitude on the dependent side, -0.25*amplitude on the opposite side, -0.3*amplitude upright, zero face up. Immediate displacement is +gravity dependent, -gravity opposite, zero otherwise. These are sensitivity assumptions, not measured tissue mechanics.');
